...
class LSTM_correlated_dropout(nn.Module):
    ...
    def __init__(self, lstm_params, is_pretraining=True):
        ...
        self.covariance_factor = self.initialize_covariance_factor()
        ...
        self.activate_noise = False
        self.is_analysing_weights = False
        self.index_weight_to_analyse = 0
        self.number_of_samples_for_training = 1
    ...
    def covariance_matrix(self):
        ...
        return self.FACTOR_TO_NORMALIZE_SOFTPLUS*activation(M).view(self.params.hidden_dim + 1,
        ...
        if self.training:
            ...
            if self.is_pretraining:
                ...
                if self.activate_noise:
                    ...
                    weights = self.weights_mu.cuda() + deviation.cuda()

                else:

                    weights = self.weights_mu

                y_pred = self.make_linear_product(weights, lstm_out)
    ...
    def initialize_covariance_factor(self):
        ...
        matrix[torch.tril(torch.ones(dimension, dimension)) == 1] = values

        return matrix.clone().detach().cuda().requires_grad_(True)
    ...
    def generate_dropout_noise(self):
        ...
        random_vector = noise_generated[:,None]*torch.sqrt(torch.diag(self.covariance_matrix)).double()
